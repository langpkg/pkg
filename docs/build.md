...
<div align="center">
    ...
    <img src="https://img.shields.io/badge/build-blue"/>
...
- ## Build
    ...
    > The `build` command compiles your TypeScript library into distributable formats using `tsup` (powered by esbuild). It generates both ESM and CommonJS outputs with TypeScript type declarations.

    ```bash
    # Basic usage
    pkg build                       # Build package with tsup

    # Options
    pkg build --watch               # Watch mode (rebuild on changes)
    pkg build --clean               # Remove dist/ before building
    ```
